...
/// Fixed-capacity fan-out bus. Each subscriber owns an independent bounded
/// ring. High-frequency non-critical events may overwrite the oldest event;
...
pub fn EventBus(comptime max_subscribers: usize, comptime queue_capacity: usize) type {
    ...
    return struct {
        ...
        pub const Subscriber = struct {
            ...
            pub fn lag(self: *const Subscriber) usize {
                return self.len;
            }
        };
        ...
        pub fn lag(self: *const Self, id: SubscriberId) usize {
            if (id >= max_subscribers) return 0;
            return self.subscribers[id].lag();
        }
    };
}
